...
function CardController.CardMove(callback)
    ...
    if (CardController.card) then
        ...
        if (CS.UnityEngine.Input.GetMouseButtonUp(0)) then
            ...
            local cardScreenPoint = CS.UnityEngine.Camera.main:WorldToScreenPoint(CardController.card.transform.position);
            -- 如果卡牌拖动位置高于屏幕2/5的高度，则判定使用了卡牌
            if(cardScreenPoint.y > threshold)then
                if(callback.UseCard) then
                    callback.UseCard(CardController.card.name);
                end
            end
            -- 卡牌移动会原位
            if(CardController.moveUtil) then
        ...
        if (CardController.selectCard ~= hitObject) then
            ...
            if (hitObject) then
                ...
                CardController.selectCard.transform.localScale = CardController.originSize * 1.5;
